...
async function sampleBasicGeneration() {
  ...
  const result = await model.generateContent({
    contents: [
      {role: 'user', parts: [{text: 'Say "Hello, World!" and nothing else.'}]},
    ],
...
async function sampleSystemInstructions() {
  ...
  const result = await model.generateContent({
    contents: [
      {role: 'user', parts: [{text: 'Tell me about Paris in 2 sentences.'}]},
    ],
    stripeCustomerId: STRIPE_CUSTOMER_ID,
  });
...
async function runAllSamples() {
  ...
  console.log(
    'These examples show how to use GoogleGenerativeAI with Stripe billing\n'
  );
